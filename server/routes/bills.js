...
router.get('/:id/download', [auth, authorize(['bills:view'])], async (req, res) => {
  try {
    ...
    doc.fillColor(colors.text)
       .fontSize(8)
       .font('Helvetica')
       .text(`Bank: ${shopProfile?.bank_name || 'N/A'}`, bankX, footerY + 16)
       .text(`Account: ${shopProfile?.account_no || 'N/A'}`, bankX, footerY + 28)
       .text(`IFSC: ${shopProfile?.ifsc || 'N/A'}`, bankX, footerY + 40);
